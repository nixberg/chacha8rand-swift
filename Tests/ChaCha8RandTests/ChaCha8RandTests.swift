...
final class ChaCha8RandTests: XCTestCase {
    ...
    func testCodableRoundtrip() throws {
        ...
        for index in Self.expectedOutput.indices {
            ...
            generator = try decoder.decode(ChaCha8Rand.self, from: encoder.encode(generator))
            for expectedValue in Self.expectedOutput.suffix(from: index) {
                XCTAssertEqual(generator.next(), expectedValue)
            }
        }
    }
    ...
    func testBinaryEncodingRoundtrip() throws {
        for index in Self.expectedOutput.indices {
            ...
            generator = try XCTUnwrap(ChaCha8Rand(decoding: generator.encoded()))
            for expectedValue in Self.expectedOutput.suffix(from: index) {
                XCTAssertEqual(generator.next(), expectedValue)
            }
        }
...
extension ChaCha8RandTests {
    ...
    fileprivate static let expectedOutput: [UInt64] = [
